...
function isCreditCardNumber(ccn) {
  ...
  for (let i = string.length - 1; i >= 0; i -= 1) {
    let digit = parseInt(string.charAt(i), 10);
    if (double) {
      digit *= 2;
      if (digit > 9) {
        digit -= 9;
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (symbol === '[' || symbol === '(' || symbol === '{' || symbol === '<') {
      ...
      if (array.length === 0 || array.pop() !== brackets[symbol]) {
        return false;
      }
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < dirs.length; i += 1) {
    const dir = dirs[i];
    const isCommon = pathes.every((path) => path.split('/')[i] === dir);
    if (!isCommon) {
      if (i === 0) {
        return '';
      }
      return `${dirs.slice(0, i).join('/')}/`;
    }
  }
